...
function format_date(df, splitter, formatx){
	var sup='', curr_date, newArrangment='', nd, split_df, m_names;
	nd = new Date();

	splitter = (splitter !== undefined) ? splitter : '-';
	formatx = (formatx !== undefined) ? formatx : 'd-m-y';
	df = (df !== undefined) ? df : nd.getDate() + splitter + nd.getMonth() + splitter + nd.getFullYear();
	split_df = df.split(splitter);
	m_names = new Array("Jan ", "Feb ", "Mar ", "Apr ", "May", "Jun ", "Jul ", "Aug ", "Sep ", "Oct ", "Nov ", "Dec ");

	$.each(formatx.split(splitter), function(i,v){
		if( split_df[i] !== undefined ){
			if( v === 'd'){
				
				curr_date = split_df[i];
				...
				newArrangment += curr_date + "<SUP>" + sup + "</SUP>";
			}

			if( v === 'm' ){
				var mx = split_df[i];
				newArrangment += m_names[unformat_money(mx)];
			}

			if( v === 'y'){
				newArrangment += split_df[i];
			}
		}
	});

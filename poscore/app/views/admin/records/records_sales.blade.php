<div class="profit-margin-bar bolder">
	...
	<span style="color: rgb(0, 206, 209); font-size: 25px;">- </span>
	<span class="">[ Cart discounts:
		<span class="orange">{{currency()}}<span class="show_total_cart_discount"></span><span>k</span></span>
	 ]</span>  = 
	<span class="">[ OVERALL SALES:
		<span class="orange">{{currency()}}<span class="overallsales"></span><span>k</span></span>
	 ]</span>

	 <span class="btn btn-minier btn-yellow more-sales-info" data-toggle="modal" role="button" data-backdrop="false">...</span>
</div>
<table id="sample-table-2" class="table table-bordered">
	...
	@if(!empty($records))
		@foreach($records as $groupBy => $v)
			<tr class="bolder tr-td-no-border">
				<td>
					<span class="bolder">{{ng_datetime_format2($v[0]['created_at'])}}</span>
				</td>
				<td><span class="bolder"><a href="#myModal" data-rel='popover' data-url={{URL::route('popoverReceiptPreview', array($groupBy))}}>{{Receipt::buildReceipt($groupBy)}}</a></span></td>
				<td colspan="6">
					<?php
						$customer =  ( $v[0]['customer'] !== null ) ? $v[0]['customer']['name'] : 'Unregisterd';
					?>
				...
				<td class="td_staffname red">
					<i class="icon-user bigger-110 hidden-phone"></i> {{User::getUsernameByID($v[0]['user_id'], false)}}
				</td>
			</tr>
			...
			@foreach($v as $history)
				<tr class='sales_infos'>
					...
					<td class="bolder force-smalltext">
						<span>{{currency()}}{{format_money($history['unitprice'])}}k</span>
					...
					<td class="td_discount">{{$history['discount']}}%</td>
					<td class="bolder force-smalltext">
			...
			@endforeach
				<tr class="transaction_detail">
					...
					<td class="bolder alert alert-info">
						...
						<div>
							<?php $receipt_worth = $v[0]['receipt']['receipt_worth']; ?>
...
$(function(){
	...
	var msi = function(e){
		...
		.find('.modal-body').html(function(){
			var show_sales_totalunitprice = unformat_money($('.show_sales_totalunitprice').text());
			var show_sales_totalcostprice = unformat_money($('.show_sales_totalcostprice').text());
			var show_sales_totalprofitmargin = unformat_money($('.show_sales_totalprofitmargin').text());
			var show_total_services = unformat_money($('.show_total_services').text());
			var show_total_cart_discount = unformat_money($('.show_total_cart_discount').text());
			var overallsales = unformat_money($('.overallsales').text());
			var totalProfits = show_sales_totalcostprice + show_total_services - show_total_cart_discount;

			return 	'<h3 class="blue nomargin bolder">Total Unit Sales: '+"{{currency()}}"+ format_money(show_sales_totalunitprice) +'k</h3>'+
					'<h4 class="red"> - Total Unit Costprice: '+"{{currency()}}"+ format_money(show_sales_totalcostprice) +'k</h4>'+
					'<h4 class="red"> - Total Unit Profits: '+"{{currency()}}"+ format_money(show_sales_totalprofitmargin) +'k</h4><hr class="nomargin">'+
					'<h3 class="blue nomargin bolder">Total Services: '+"{{currency()}}"+ format_money(show_total_services) +'k</h3><hr class="nomargin">'+
					'<h3 class="blue nomargin bolder">Total Discounted: '+"{{currency()}}"+ format_money(show_total_cart_discount) +'k</h3><hr class="nomargin">'+
					'<h3 class="blue nomargin bolder">Total Overall Sales: '+"{{currency()}}"+ format_money(overallsales) +'k</h3><hr class="nomargin">'+
					'<h5 class="muted nomargin bolder"> Total Units profit + Total Services - Total Discounted</h5>'+
					'<h2 class="bolder green nomargin"> Total Profit: '+"{{currency()}}"+ format_money(totalProfits) +'k</h2>';
		})
		.end()
		.find('.modal-header')
		.css({'background':'grey'})

		...
		$('.editable-controller').on('click', function(e){
			...
			$(that).editable({
				display:function(value){
					if( $.trim(value) !== '' ){
						var $el = $('[editable-view-id="'+editable_id+'"]');

						var elUrl = $el.prop('href');
						var getLast = elUrl.split('/').pop();
						var newValue = value.replace(/[^a-z0-9 \s+]+/gmi, '').toLowerCase();

						var elnewUrl = elUrl.replace(getLast, newValue.replace(/\s+/g, '+'));

						$el.prop('href', elnewUrl);

						$el.text(newValue).css({'text-transform':'capitalize'});
					}
				},
				emptytext:'',
				type: 'text',
...
/*********** EDITABLE ENDS HERE  *********/
		...
		$('.single_delete, .multiple_delete').deleteItemx({
			...
			confirmationMessage: "<i class='icon-exclamation red'></i> You're about to delete <b>[ :nameholder ]</b> and all it's categories and products. This action can not be reversed.  Are you sure?"
		});

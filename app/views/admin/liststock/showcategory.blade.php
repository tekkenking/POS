		...
		@if($counter <= 4)
			<div class="span3 widget-container-span deletethis">
				<div class="widget-box light-border">
					<div class="">
						<h3 class="smaller @if($cat['type'] == 'service') blue @else green @endif" style="margin:0">
							<span class="editable-view" editable-view-id="{{$cat['id']}}">
								<a href={{URL::route('adminShowProduct', array(slug($brand->name), slug($cat['name'])) )}} class="nolinkdecoration">{{{ucfirst($cat['name'])}}}</a>
							</span>
						</h3>
					</div>
		...
		$('.editable-controller').on('click', function(e){
			...
			$(that).editable({
				display:function(value){
					if( $.trim(value) !== '' ){
						var pplink = "{{Request::url()}}/" + value.toLowerCase().replace(/[ \t\r]+/g, "+");
